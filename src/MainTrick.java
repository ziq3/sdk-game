...
public class Main {
    ...
    private static final String GAME_ID = "184746";
    ...
    public static void main(String[] args) throws IOException {
        ...
        Emitter.Listener onMapUpdate = new Emitter.Listener() {
            ...
            int getPointArmor(Armor armor) {
                ...
                return gainArmor * 600 / (distance(armor) + 1);
            }

            int getPointHealth(int health) {
                health = Math.min(health, Math.max(100 - me.getHp(), 50));
            ...
            int getPointWeapon(Weapon weapon) {
                ...
                if (weapon.getType() == ElementType.GUN) {
                    pointWeapon = (weapon.getDamage() * weapon.getCapacity() - Utils.getDame(gun) * me.getBulletNum())
                            * 2;
                }
            ...
            void calculateOptimizedMove() {
                ...
                for (Player p : otherPlayers) {
                    ...
                    for (int i = 0; i < 4; ++i) {
                        Node diffNode;
                        if (haveGun) {
                            diffNode = Utils.multiply(DIRECTIONS.get(i), 4);
                        } else {
                            diffNode = DIRECTIONS2.get(i);
                        }
                        Node addNode = Utils.add(p, diffNode);
